/**
 * FILE: services/interview/controllers/interview.controller.js
 * PURPOSE: Exposes REST API endpoints for starting interviews, submitting answers, and fetching histories.
 * Crucially, it translates HTTP requests into LangGraph state machine actions ("start", "feedback").
 */
    ...
    try {
        ...
        if (githubUrl) {
            try {
                ...
                if (username) {
                    const ghRes = await fetch(`https://api.github.com/users/${username}/repos?sort=updated&per_page=3`);
                    if (ghRes.ok) {
                        const repos = await ghRes.json();
                        githubData = repos.map(r => `${r.name} (${r.language || "Mixed"}): ${r.description || "No description"}`).join(" | ");
                    }
                }
            } catch (err) {
